...
export function ScreenDesignPage() {
  ...
  return (
    <div className="h-screen bg-[#0B0D14] animate-fade-in flex flex-col overflow-hidden">
      {/* Header */}
      <header className="border-b border-white/[0.06] bg-background/90 backdrop-blur-sm shrink-0 z-50">
        <div className="px-4 py-2 flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate(`/sections/${sectionId}`)}
            className="text-white/65 hover:text-white -ml-2"
          >
          ...
          <div className="flex items-center gap-2 min-w-0">
            <Layout className="w-4 h-4 text-[#a0b4ff] shrink-0" strokeWidth={1.5} />
            {section && (
      ...
      >
        ...
        >
          <div className="w-1 h-16 rounded-full bg-white/15 group-hover:bg-white/30 transition-colors flex items-center justify-center">
            <GripVertical className="w-3 h-3 text-white/40 opacity-0 group-hover:opacity-100 transition-opacity" strokeWidth={2} />
          </div>
        </div>

        {/* Preview container using iframe for true isolation */}
        <div
          className="bg-white dark:bg-stone-950 rounded-lg shadow-xl border border-white/[0.06] overflow-hidden"
  ...
  if (!ScreenDesignComponent) {
    return (
      <div className="h-screen flex items-center justify-center bg-background">
        <p className="text-white/65">Tela não encontrada.</p>
      </div>
    )
  }
