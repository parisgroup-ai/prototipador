...
const phases: PhaseConfig[] = [
  { id: 'product', label: 'Produto', icon: FileText, path: '/' },
  { id: 'data-shape', label: 'Dados', icon: Boxes, path: '/data-shape' },
  { id: 'design', label: 'Design', icon: Layout, path: '/design' },
  { id: 'sections', label: 'Seções', icon: LayoutList, path: '/sections' },
  { id: 'export', label: 'Exportar', icon: Package, path: '/export' },
]
          ...
          <div key={phase.id} className="flex items-center">
            ...
            {!isFirst && (
              <div
                className={`w-4 sm:w-8 lg:w-12 h-px transition-colors duration-200 ${
                  status === 'upcoming'
                    ? 'bg-white/[0.08]'
                    : 'bg-gradient-to-r from-[#4F6AFF]/60 to-[#8B5CF6]/60'
                }`}
              />
            )}
            ...
            <button
              ...
              className={`
                ...
                ${status === 'current'
                  ? 'paris-cta text-white'
                  : status === 'completed'
                    ? 'bg-white/[0.04] text-white/85 hover:bg-white/[0.08] border border-white/[0.06]'
            ...
            >
              ...
              {isComplete && (
                <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-gradient-to-br from-[#4F6AFF] to-[#8B5CF6] flex items-center justify-center shadow-sm shadow-[#8B5CF6]/40">
